# Info:
# This node does a breadth search of the pointcloud, doing a quick check of every message. 
# It checks for identical frames, deviations in point count, and message rate.
...
class LidarBreadthSearch(Node):
    def __init__(self):
        super().__init__('lidar_breadth_search')
...
def main(args=None):
    print("Starting LiDAR Breadth Search Node")
    rclpy.init(args=args)
    node = LidarBreadthSearch()

    print("Node initialized, spinning now.")
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
    print("LiDAR Breadth Search Node has been shut down.")
